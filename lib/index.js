...
const Popover = createClass({
  ...
  resolvePopoverLayout () {
    ...
    let tipShadowWidth = Math.pow(this.props.tipSize * 2, 2)
    tipShadowWidth = Math.ceil(Math.sqrt(tipShadowWidth / 2))
    this.tipShadowEl.style.width = `${tipShadowWidth}px`
    this.tipShadowEl.style.height = `${tipShadowWidth}px`

    const margin = Math.ceil((this.props.tipSize * 2 - tipShadowWidth) / 2)
    if (zone.flow === "column") {
      this.tipShadowEl.style.left = `${tipCrossPos + margin}px`
      if (zone.standing === "below") {
        this.tipShadowEl.style.bottom = `-${this.props.tipSize - margin}px`
      } else {
        this.tipShadowEl.style.top = `-${this.props.tipSize - margin}px`
      }
    } else if (zone.standing === "right") {
      this.tipShadowEl.style.top = `${tipCrossPos + 1 + margin}px`
      this.tipShadowEl.style.right = `-${this.props.tipSize - margin}px`
    } else {
      this.tipShadowEl.style.top = `${tipCrossPos + margin}px`
      this.tipShadowEl.style.left = `-${this.props.tipSize - margin}px`
    }
  },
  ...
  renderLayer () {
    ...
    return (
      E.div(popoverProps,
        E.div({ className: "Popover-body", style: { zIndex: 2 }}, ...popoverBody),
